...
#include <queue> //to allow the use of queue functions
#include <algorithm> //to allow the use min()
...
/**
 * @brief Swap spots in the vector.
 * 
 * @param vec 
 * @param i 
 * @param j 
 ...
 * @return int 
 */
int partitionNodes(std::vector<Node*> &vec, int low, int high) {
    int pivot = vec[high]->sum;
    int i = (low - 1);
 ...
 * @param high 
 */
void quickSortNodes(std::vector<Node*> &vec, int low, int high) {
    if (low < high) {
        int pi = partitionNodes(vec, low, high);
        quickSortNodes(vec, low, pi - 1);
        quickSortNodes(vec, pi + 1, high);
    }
} //quickSortNodes
...
std::vector<std::vector<char> > GetIntByMatrixSum ( std::vector<std::vector<int> > mat, std::vector<std::vector<char> > pathMap, int n) {
    ...
    std::vector<Node*> allNodes; //a vector of nodes to keep track of the path
    std::queue<Node*> queue; //a queue of nodes to keep track of the path
    std::vector<Node *> children; //a vector of nodes to keep track of the path
    for (int i = 0; i < mat.size(); i++)
    { // iterate over rows
        for (int j = 0; j < mat[i].size(); j++) {
            Node* curr = new Node(i, j, mat[i][j], nullptr); //create a new node
            children.push_back(curr); //add the node to the vector of all nodes
        }
    }
    while (!children.empty()) {
        quickSortNodes(children, 0, children.size() - 1);
        int pathCountThreshold = std::min(int((mat.size()+mat[0].size())/2), int(children.size()));
        for (int i = 0; i < pathCountThreshold ; i++) {
            queue.push(children[children.size()-i-1]);
        }
        children.erase(children.end()-pathCountThreshold, children.end());
        clearMem(children);
        children.clear();
        while (!queue.empty())
        {
            Node *curr = queue.front(); // get top of queue
            queue.pop();                // pop top of queue
            if (curr->sum == n) {       // check if sum is equal to n
                ...
                clearMem(allNodes);      // clear memory
                return pathMap;
            } // if
            else if (curr->sum < n) {
                int dx[4] = {0, 0, 1, -1};
